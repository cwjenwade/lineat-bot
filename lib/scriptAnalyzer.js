...
function mapSpeakerToCharacterId(speaker, characters = []) {
  if (!speaker) return '';
  const normalized = speaker.replace(/[（(].*?[）)]/g, '').trim();
  const matched = characters.find((character) => normalized.includes(character.name) || character.name.includes(normalized));
  return matched?.id || '';
}

function buildSuggestion(section, index, characters = []) {
  ...
  const speakerCharacterId = mapSpeakerToCharacterId(speaker, characters);
  ...
  let type = dialogueLines.length ? 'dialogue' : 'narrative';
  if (!dialogueLines.length && paragraphs.length <= 1 && text.length <= 120) {
    type = 'fullscreen';
  }
...
function analyzeScript(script, existingNodes = [], characters = []) {
  const normalized = normalizeScript(script);
  const title = extractTitle(normalized);
  const sections = splitByPic(normalized);
  const suggestedNodes = sections.map((section, index) => buildSuggestion(section, index, characters));
