...
function createStoryRuntime(options = {}) {
  ...
  async function processTextInput(textOrInput, sessionKey = 'local-user', runtimeOptions = {}) {
    ...
    if (STORY_ENTRY_ALIASES.has(trimmed)) {
      const baseUrl = derivedRuntimeOptions.publicBaseUrl || publicBaseUrl;
      const availableStories = (store.stories || []).filter((story) => story?.startNodeId);
      const defaultStoryId = `${derivedRuntimeOptions.defaultStoryId || process.env.DEFAULT_STORY_ID || 'example'}`.trim();
      const targetUrl = availableStories.length && defaultStoryId
        ? buildStoryEntryUrl(baseUrl, defaultStoryId)
        : buildStoryEntryUrl(baseUrl, '');
