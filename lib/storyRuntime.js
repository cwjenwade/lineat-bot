...
function createStoryRuntime(options = {}) {
  ...
  async function buildReplyForTrigger(store, binding, sessionKey, runtimeOptions = {}) {
    ...
    const render = await buildRenderResult(store, story, nodeId, baseUrl, buildRenderOptions(runtimeOptions, baseUrl));
  ...
  async function buildReplyForSession(store, session, sessionKey, text, runtimeOptions = {}) {
    ...
    if (resolution.action === 'feedback') {
      const current = await buildRenderResult(store, story, node.id, baseUrl, buildRenderOptions(runtimeOptions, baseUrl));
    ...
    if (resolution.action === 'advance') {
      ...
      const next = await buildRenderResult(store, story, resolution.nextNodeId, baseUrl, buildRenderOptions(runtimeOptions, baseUrl));
      const virtualTransition = resolution.feedback
        ? [{
  ...
  async function processTextInput(text, sessionKey = 'local-user', runtimeOptions = {}) {
    ...
    if (source === 'preview') {
      ...
      const baseUrl = runtimeOptions.publicBaseUrl || publicBaseUrl;
      const render = await buildRenderResult(store, story, node.id, baseUrl, buildRenderOptions(runtimeOptions, baseUrl));
      return {
        source,
        mode: 'preview',
        messages: render.payload.messages,
