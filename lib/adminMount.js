...
function createApiRouter() {
  ...
  async function buildRenderPreviewResponse(req) {
    ...
    const activeNode = activeStory.nodes.find((entry) => entry.id === (req.body?.nodeId || activeStory.startNodeId));
    if (!activeNode) {
      const error = new Error('Node not found');
      error.status = 404;
      throw error;
    }
    const render = await renderNode(activeStore, activeStory, activeNode, getLineBaseUrl(), {
    ...
    try {
      ...
      res.json({
        published: {
          ...
          deletedAssetCount: published.deletedAssetPaths.length,
  ...
  router.post('/stories/:storyId/test/trigger', requireManager, async (req, res) => {
    ...
    try {
      ...
      const render = await buildRenderResult(store, story, startNodeId, getLineBaseUrl(), {
        requirePublishedAssets: false,
        usePublishedAssets: false
      });
      const validation = await validateMessagesWithLine(render.payload.messages, process.env.LINE_CHANNEL_ACCESS_TOKEN, 'broadcast');
