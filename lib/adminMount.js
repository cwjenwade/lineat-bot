...
function createApiRouter() {
  ...
  async function buildRenderPreviewResponse(req) {
    ...
    const result = await localRuntime.processTextInput('', `${req.actorMeta.actor}:preview:${activeStory.id}:${activeNode.id}`, {
      source: 'preview',
  ...
  router.delete('/stories/:storyId', (req, res) => {
    try {
      ...
      const latestStore = readStore();
      const nextStory = latestStore.stories[Math.min(storyIndex, Math.max(0, latestStore.stories.length - 1))] || null;
      res.json({
