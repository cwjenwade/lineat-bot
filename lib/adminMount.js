...
const { hasCloudinaryConfig, uploadStoryNodeImage } = require('./cloudinaryClient');
const {
...
function getLineBaseUrl() {
  return process.env.PUBLIC_BASE_URL || 'https://debbylinehose.vercel.app';
}
...
function createApiRouter() {
  ...
  router.post('/stories', requireManager, (req, res) => {
    try {
  ...
  router.put('/stories/:storyId', requireManager, (req, res) => {
    try {
  ...
  router.post('/stories/:storyId/duplicate', requireManager, (req, res) => {
    try {
      const store = readStore();
  ...
  router.delete('/stories/:storyId', requireManager, (req, res) => {
    try {
      const store = readStore();
  ...
  router.post('/stories/:storyId/import-script', requireManager, (req, res) => {
    const store = readStore();
  ...
  router.post('/stories/:storyId/import-file', requireManager, upload.single('file'), (req, res) => {
    const store = readStore();
    const story = store.stories.find((entry) => entry.id === req.params.storyId);
  ...
  router.put('/stories/:storyId/draft', requireManager, (req, res) => {
    const story = getStory(req.params.storyId);
  ...
  router.post('/stories/:storyId/draft/reparse-node', requireManager, (req, res) => {
    const store = readStore();
    const story = store.stories.find((entry) => entry.id === req.params.storyId);
  ...
  router.post('/stories/:storyId/nodes/:nodeId/upload-image', requireManager, async (req, res) => {
    if (!hasCloudinaryConfig()) {
      return res.status(500).json({ error: 'Missing Cloudinary env' });
    }
    ...
    const node = story.nodes.find((entry) => entry.id === req.params.nodeId);
    ...
    try {
      const uploaded = await uploadStoryNodeImage({
        buffer: req.file.buffer,
        storyId: story.id,
        nodeId: node.id
      });

      node.imageUrl = uploaded.secureUrl;
      node.imageMeta = {
        provider: 'cloudinary',
      ...
      const saved = saveStory(story.id, story, req.actorMeta);
      const updatedNode = (saved.nodes || []).find((entry) => entry.id === node.id) || node;

      recordAction('story.node.upload-image', {
        actor: req.actorMeta.actor,
        role: req.actorMeta.role,
        targetId: `${story.id}:${node.id}`,
        result: 'success',
        detail: JSON.stringify({ provider: 'cloudinary', publicId: uploaded.publicId })
      });

      return res.status(201).json({ node: updatedNode, story: saved });
    } catch (error) {
      recordAction('story.node.upload-image', {
        actor: req.actorMeta.actor,
        role: req.actorMeta.role,
