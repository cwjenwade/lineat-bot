...
function createApiRouter() {
  ...
  router.post('/stories/:storyId/render/node', async (req, res) => {
    ...
    try {
      const render = await buildRenderResult(store, story, req.body?.nodeId || story.startNodeId, getPublicBaseUrl(req));
      res.json({ render });
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
  ...
  router.post('/stories/:storyId/render/draft', async (req, res) => {
    ...
    try {
      ...
      const render = await buildRenderResult(draftStore, draftStory, req.body?.nodeId || draftStory.startNodeId, getPublicBaseUrl(req), {
  ...
  router.post('/stories/:storyId/render/story', async (req, res) => {
    ...
    try {
      const chain = await Promise.all(
        story.nodes.map((node) => buildRenderResult(store, story, node.id, getLineBaseUrl(), {
  ...
  router.post('/stories/:storyId/publish-assets', requireManager, async (req, res) => {
    ...
    try {
      ...
      const saved = saveStory(story.id, {
        ...story,
      ...
      recordAction('render.publish-assets', {
        ...
        targetId: story.id,
        result: 'success',
        detail: JSON.stringify({ assetCount: published.assetPaths.length })
      });
      res.json({
        published: {
          storyId: story.id,
          nodeCount: story.nodes.length,
          assetCount: published.assetPaths.length,
          assets: published.assetPaths.map((assetPath) => resolveAssetUrl(assetPath, getPublicBaseUrl(req))),
  ...
  router.post('/stories/:storyId/validate/node', async (req, res) => {
    ...
    try {
      const render = await buildRenderResult(store, story, req.body?.nodeId || story.startNodeId, getLineBaseUrl(), {
  ...
  router.post('/stories/:storyId/test/node', requireManager, async (req, res) => {
    ...
    try {
      const published = await publishStoryAssets(store, story, getPublicBaseUrl(req));
      const publishedStory = saveStory(story.id, {
        ...story,
        publishedAssets: published.publishedAssets
      }, req.actorMeta);
      const render = await buildRenderResult(readStore(), publishedStory, req.body?.nodeId || publishedStory.startNodeId, getLineBaseUrl(), {
        requirePublishedAssets: true
      });
  ...
  router.post('/stories/:storyId/test/trigger', requireManager, async (req, res) => {
    ...
    try {
      ...
      const currentStory = currentStore.stories.find((entry) => entry.id === publishedStory.id) || publishedStory;
      const startNodeId = req.body?.nodeId || currentStory.startNodeId;
      const render = await buildRenderResult(currentStore, currentStory, startNodeId, getLineBaseUrl(), {
        requirePublishedAssets: true
      });
      const validation = await validateMessagesWithLine(render.payload.messages, process.env.LINE_CHANNEL_ACCESS_TOKEN, 'broadcast');
      if (!validation.ok) {
