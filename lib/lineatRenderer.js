...
function createTextSvg({
  ...
  const blockHeight = Math.max(fontSize, lines.length * computedLineHeight - lineSpacing);
  const startY = y + Math.max((height - blockHeight) / 2, 0) + fontSize * 0.88;
...
async function buildDialogueOrNarrationCardBuffer(model, globalSettings) {
  ...
  for (const [index, role] of roles.entries()) {
    ...
    const avatar = await extractCoverBuffer(role.avatarPath || role.avatarUrl, avatarSize, avatarSize, {
...
async function buildChoiceCardBuffer(model) {
  ...
  const height = layout.totalHeight * scale;
  const heroHeight = layout.heroHeight * scale;
  const questionHeight = layout.questionHeight * scale;
  const actionsHeight = layout.actionsHeight * scale;
  const questionTop = heroHeight;
  const actionsTop = heroHeight + questionHeight;
  const sidePadding = layout.bodyPaddingSide * scale;
  const gap = layout.buttonSpacing * scale;
  const hero = await extractCoverBuffer(model.imagePath || model.imageUrl, width, heroHeight, {
    centerX: 50,
    centerY: 50,
    zoom: Number.isFinite(model.heroImageScale) ? model.heroImageScale : 1
  ...
  const promptFontSize = 20 * scale;
  const promptSvg = createTextSvg({
    x: sidePadding,
    y: questionTop + (18 * scale),
    width: width - (sidePadding * 2),
    height: questionHeight - (36 * scale),
    text: model.prompt,
  ...
  const buttonHeight = Math.round((actionsHeight - (gap * 3)) / 2);
  const buttonWidth = width - (sidePadding * 2);
  const buttonAX = sidePadding;
  const buttonAY = actionsTop + gap;
  const buttonBY = buttonAY + buttonHeight + gap;
  const buttonFontSize = 18 * scale;

  const svg = `
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      <rect x="0" y="${questionTop}" width="${width}" height="${questionHeight}" fill="#FFF8EF" />
      <rect x="0" y="${actionsTop}" width="${width}" height="${actionsHeight}" fill="#FFFDF8" />
      ${promptSvg}
      <rect x="${buttonAX}" y="${buttonAY}" width="${buttonWidth}" height="${buttonHeight}" rx="${16 * scale}" ry="${16 * scale}" fill="#F3BD63" />
      <rect x="${buttonAX}" y="${buttonBY}" width="${buttonWidth}" height="${buttonHeight}" rx="${16 * scale}" ry="${16 * scale}" fill="#D8E0EF" />
      ${createTextSvg({ x: buttonAX + (24 * scale), y: buttonAY, width: buttonWidth - (48 * scale), height: buttonHeight, text: model.optionA.label, fontSize: buttonFontSize, lineSpacing: 6 * scale, color: '#2D241B', weight: 800 })}
      ${createTextSvg({ x: buttonAX + (24 * scale), y: buttonBY, width: buttonWidth - (48 * scale), height: buttonHeight, text: model.optionB.label, fontSize: buttonFontSize, lineSpacing: 6 * scale, color: '#2D241B', weight: 800 })}
    </svg>
  `;
...
function createChoiceQuickReply(choiceModel) {
  return {
    type: 'text',
...
async function resolveModelImage(model, globalSettings, story, publicBaseUrl, options = {}) {
  const publishedPath = resolvePublishedImagePath(story, model);
  if (publishedPath) {
    return {
...
async function buildRenderResult(store, story, nodeId, publicBaseUrl, options = {}) {
  ...
  if (choiceModel) {
    lineMessages.push(createRenderedImageBubble(
      choiceModel.renderedImageUrl,
      choiceModel.prompt || '故事選項'
    ));
    lineMessages.push(createChoiceQuickReply(choiceModel));
...
function evaluateStoryIssues(story, store) {
  ...
  story.nodes.forEach((node) => {
    ...
    const unpublished = buildNodeModels(story, node, globalSettings, '')
      .filter((model) => {
        const publishedPath = story.publishedAssets?.[model.renderKey];
        return !publishedPath || !assetExists(publishedPath);
      });
    if (unpublished.length) {
      issues.push({
        ...
        scope: 'node',
        nodeId: node.id,
        field: 'publishedAssets',
        message: `尚未發布部署圖片 (${unpublished.length} 張)`
      });
    }
  });
