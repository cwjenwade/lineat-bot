...
// Defer loading storyData until createSeedStory to avoid heavy startup parsing
let storyNodes = null;
let storyStartId = null;
    ...
    characters: [
      {
        ...
        sortOrder: 1,
        avatarPath: '/public/story/01/roles/bhead001.png',
        placement: 'left',
        avatarX: 14,
        avatarY: 332,
        avatarSize: 92,
...
function createSeedStory() {
  if (!storyNodes) {
    const sd = require('../storyData');
    storyNodes = sd.storyNodes;
    storyStartId = sd.storyStartId;
  }
  ...
  seed.nodes = (Array.isArray(storyNodes) ? storyNodes : []).map((node, index) => {
    const order = index + 1;
